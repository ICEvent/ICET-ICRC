...
persistent actor class ICETToken(initialOwner : Principal, initialSupply : Nat) = this {
  ...
  // Absorbs the old stable variable from a previous deployment; no longer used.
  var chainsEntries : [(Text, Nat)] = [];
  ...
  // ── Old-ICET migration via EXT transfer notification ────────────────────
  //
  // The old ICET (ot4zw-oaaaa-aaaag-qabaa-cai) uses the EXT standard.
  // When a user calls transfer() on the old canister with notify:true and
  // to:#principal(<this canister>), the old canister calls
  // tokenTransferNotification() here with the amount the user chose.
  // New ICET is minted 1:1 to that user immediately.
  //
  // Migration workflow (call on the OLD canister):
  //   transfer({
  //     token      = "<old canister token id>";
  //     from       = #principal(<your principal>);
  //     to         = #principal(<this canister id>);
  //     amount     = <N>;          // ← user specifies this
  //     memo       = "";
  //     notify     = true;
  //     subaccount = null;
  //   })

  transient let OLD_ICET_CANISTER : Principal =
    Principal.fromText("ot4zw-oaaaa-aaaag-qabaa-cai");

  // EXT User variant used by the old canister
  type ExtUser = {
    #address : Text;     // AccountIdentifier (hex-encoded)
    #principal : Principal;
  };

  /// Called automatically by the old ICET canister after a user's transfer
  /// with notify:true.  Mints new ICET 1:1 to the sender.
  /// Returns ?amount to confirm (returning null rolls back the transfer).
  public shared ({ caller }) func tokenTransferNotification(
    _tokenId : Text,
    from     : ExtUser,
    amount   : Nat,
    _memo    : Blob,
  ) : async ?Nat {
    if (caller != OLD_ICET_CANISTER) { return null };
    if (amount == 0) { return null };

    switch (from) {
      case (#principal(p)) {
        let toAccount : Account = { owner = p; subaccount = null };
        putBalance(toAccount, getBalance(toAccount) + amount);
        totalSupply += amount;
        ?amount;
      };
      // #address senders cannot be reverse-mapped to a principal; reject.
      case (#address(_)) { null };
    };
  };
};
